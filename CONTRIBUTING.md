...
- You can run `make backenddocs` to verify the resulting Markdown.
  ...
  - **Do not** commit the updated Markdown files.
    This operation is run as part of
...
- Try to implement as many optional methods as possible as it makes the remote
  ...
  - Use [lib/encoder](https://pkg.go.dev/github.com/rclone/rclone/lib/encoder) to
    ...
    - `rclone test info --all --remote-encoding None -vv --write-json
      remote.json TestRemote:rclone-info`
    - `go run cmd/test/info/internal/build_csv/main.go -o remote.csv
      remote.json`
