...
LDFLAGS=--ldflags "-s -X /fs.Version=$(TAG)"
...
test_all:
	go install $(LDFLAGS) $(BUILDTAGS) $(BUILD_ARGS) /fstest/test_all
...
btest:
	@echo "[$(TAG)]($(BETA_URL)) on branch [$(BRANCH)](https:///tree/$(BRANCH)) (uploaded in 15-30 mins)" | xclip -r -sel clip
	@echo "Copied markdown of beta release to clip board"

btesth:
	@echo "<a href="$(BETA_URL)">$(TAG)</a> on branch <a href="https:///tree/$(BRANCH)">$(BRANCH)</a> (uploaded in 15-30 mins)" | xclip -r -sel clip -t text/html
...
# Quick test
quicktest:
	SCE_CONFIG="/notfound" go test $(LDFLAGS) $(BUILDTAGS) ./...

racequicktest:
	SCE_CONFIG="/notfound" go test $(LDFLAGS) $(BUILDTAGS) -cpu=2 -race ./...

compiletest:
	SCE_CONFIG="/notfound" go test $(LDFLAGS) $(BUILDTAGS) -run XXX ./...
