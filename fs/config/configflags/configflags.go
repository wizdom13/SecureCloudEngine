...
func AddFlags(ci *fs.ConfigInfo, flagSet *pflag.FlagSet) {
	...
	flags.StringVarP(flagSet, &cacheDir, "cache-dir", "", config.GetCacheDir(), "Directory sce will use for caching", "Config")
	flags.StringVarP(flagSet, &tempDir, "temp-dir", "", os.TempDir(), "Directory sce will use for temporary files", "Config")
